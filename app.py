...
# --- CSS HACK TO HIDE CHECKBOX COLUMN ---
st.markdown("""
    <style>
    /* Hide the selection checkbox column in dataframes */
    [data-testid="stMain"] [data-testid="stTable"] td:first-child,
    [data-testid="stMain"] [data-testid="stTable"] th:first-child {
        display: none;
    }
    /* This target specifically hides the interactive dataframe selection column */
    [data-testid="stDataFrameDataLayer"] > div:first-child {
        display: none;
    }
    </style>
...
try:
    ...
    if selected_sales != "-- All Sales --":
        ...
        # Whole row remains clickable, but CSS hides the checkbox
        event = st.dataframe(
            summary_table, 
